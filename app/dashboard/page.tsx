    ...
    <div className="d-flex flex-column gap-4">
      ...
      {toast && (
        <div className="position-fixed top-0 start-50 translate-middle-x mt-4" style={{ zIndex: 9999 }}>
          <div className="alert shadow-lg rounded-3 px-4 py-3 d-flex align-items-center gap-3" style={{ minWidth: "300px", background: toast.includes("??") ? "#fef2f2" : "#d1fae5", border: toast.includes("??") ? "1px solid #fecaca" : "1px solid #86efac" }}>
            <span style={{ fontSize: 24 }}>{toast.includes("??") ? "??" : "?"}</span>
            <span className="fw-semibold" style={{ color: toast.includes("??") ? "#dc2626" : "#059669" }}>{toast}</span>
          </div>
        </div>
      )}
