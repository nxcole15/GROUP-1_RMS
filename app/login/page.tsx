...
function detectRole(id: string) {
  const value = normalizeId(id);
  if (!value) return "";
  if (/^[0-9]{8,12}$/.test(value)) return "Student";
  if (/^[T]/.test(value)) return "Teacher";
  if (/^[R]/.test(value)) return "Registrar";
  if (/^[P]/.test(value)) return "Principal";
  if (/^[D]/.test(value)) return "Dean";
  if (/^[A]/.test(value)) return "Accounting";
...
export default function LoginPage() {
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setForm({ ...form, [name]: name === "id" ? value.toUpperCase() : value });
    setError("");
  }
  ...
  function handleForgotPasswordSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (forgotPasswordStep === "email") {
      // Step 1: Verify email and user ID
      ...
      console.log(`Password reset code for ${account.name}: ${code}`);
      ...
      const normalizedUserId = normalizeId(forgotPasswordForm.userId);
      const accountIndex = USER_ACCOUNTS.findIndex((acc) => 
        acc.id === normalizedUserId || acc.altIds?.map((a) => a.toUpperCase()).includes(normalizedUserId)
      );

      if (accountIndex !== -1) {
        USER_ACCOUNTS[accountIndex].password = forgotPasswordForm.newPassword;
        console.log(`Password updated for ${USER_ACCOUNTS[accountIndex].name}`);
      }
          ...
          <div className="row justify-content-center">
            <div className="col-12 col-md-8 col-lg-6 col-xl-5">
              ...
              <div className="text-center mb-5">
                <h1 className="display-4 fw-extrabold mb-3" style={{ color: "#dc2626" }}>Login</h1>
              ...
              <div className="bg-white rounded-4 shadow-lg p-5" style={{ border: "1px solid #fbbf24" }}>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  {detectedRole && (
                    <div className="mb-4 rounded-xl p-3" style={{ background: "#fffbeb", border: "1px solid #fde68a", color: "#92400e" }}>
            ...
            <div className="p-4">
              {forgotPasswordStep === "email" && (
                <form onSubmit={handleForgotPasswordSubmit}>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      type="text"
                      value={forgotPasswordForm.userId}
                      onChange={(e) => setForgotPasswordForm({ ...forgotPasswordForm, userId: e.target.value.toUpperCase() })}
                      placeholder="e.g. 202400001, T001, R001"
                      className="form-control form-control-lg rounded-xl"
                      style={{ borderColor: "#f97316" }}
                      required
                    />
                  ...
                  {forgotPasswordError && (
                    <div className="alert py-2 px-3 rounded-xl mb-3" style={{ background: "#fef2f2", borderColor: "#dc2626", color: "#dc2626", fontSize: "14px" }}>
                      {forgotPasswordError}
                    </div>
                  )}

                  {forgotPasswordSuccess && (
                    <div className="alert py-2 px-3 rounded-xl mb-3" style={{ background: "#f0fdf4", borderColor: "#10b981", color: "#059669", fontSize: "14px" }}>
                      {forgotPasswordSuccess}
                    </div>
                  )}

                  <button type="submit" className="btn btn-lg w-100 fw-semibold py-3 rounded-xl" style={{ background: "linear-gradient(135deg, #dc2626, #f97316)", color: "white" }}>
                ...
                <form onSubmit={handleForgotPasswordSubmit}>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e) => setForgotPasswordForm({ ...forgotPasswordForm, code: e.target.value.replace(/\D/g, "").slice(0, 6) })}
                      placeholder="000000"
                      className="form-control form-control-lg text-center font-monospace rounded-xl"
                      style={{ borderColor: "#f97316", fontSize: "24px", letterSpacing: "0.5em" }}
                      maxLength={6}
                      required
                    />
                    <div className="form-text text-center mt-2">
                      <small className="text-muted">Demo: Your code is <strong className="text-success">{generatedCode}</strong></small>
                    </div>
                  </div>
                  ...
                  <button type="submit" className="btn btn-lg w-100 fw-semibold py-3 rounded-xl mb-2" style={{ background: "linear-gradient(135deg, #dc2626, #f97316)", color: "white" }}>
                    Verify Code
                  </button>
                  
                  <button type="button" onClick={() => setForgotPasswordStep("email")} className="btn btn-link w-100 text-decoration-none" style={{ color: "#f97316" }}>
                    ← Back to Email
                  </button>
                </form>
              )}
              ...
              {forgotPasswordStep === "success" && (
                <div className="text-center py-4">
                  ...
                  <button onClick={resetForgotPasswordModal} className="btn btn-lg w-100 fw-semibold py-3 rounded-xl" style={{ background: "linear-gradient(135deg, #dc2626, #f97316)", color: "white" }}>
                    Go to Login
                  </button>
                </div>
              )}
